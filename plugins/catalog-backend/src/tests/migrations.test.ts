...
describe.each(databases.eachSupportedId())('migrations, %p', databaseId => {
  ...
  it('20241111000000_drop_redundant_indices.js', async () => {
    ...
    await migrateUntilBefore(knex, '20241111000000_drop_redundant_indices.js');
  ...
  it('20260215000000_move_stitch_queue.js', async () => {
    ...
    const revertedRefreshColumnInfo = await knex('refresh_state').columnInfo();
  ...
  it('20260403000000_add_location_entity_ref.js', async () => {
    ...
    await migrateUntilBefore(knex, '20260403000000_add_location_entity_ref.js');
